...
var files = {
	root: ["package.json", ".gitignore", ".editorconfig", "test/module.spec.js", ".jshintrc"]
};
...
describe("When executing Yeoman with a default configuration", function () {
	let tmpFolder = os.tmpdir()+"/gen-fidos-" + randoms();
	...
	it("Should create files in templates root", function(done){
		...
		execFile("yo", ["fidos:module","--usefile"], {timeout: 10000},  (err, stdout, stderr) => {
			console.log(err);
			console.log(stdout);
			console.log(stderr);
